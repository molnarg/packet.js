...
function nestedStructure(structures, name, prototype, previous) {
  ...
  properties[name + '__offset'] = { get: function() { return this[name][name + '__offset'] } }
  properties[name + '__length'] = { get: function() { return this[name][name + '__length'] } }
...
function structureProperty(desc, name, prototype, previous) {
  ...
  var properties = {}, offset, offset_property, length, length_property

  // Offset depends on the previous property or it is given explicitly
  offset_property = desc.offset ? propertyExpression(desc.offset) : {
    get: function() { return this[previous + '__offset'] + this[previous + '__length'] }
  }
  try { offset = offset_property.value || offset_property.get.call(prototype) } catch(e) { offset = undefined }
  if (typeof offset === 'number' && !isNaN(offset)) offset_property = { value: offset }
  Object.defineProperty(prototype, name + '__offset', offset_property)
  properties[name + '__offset'] = offset_property  // TODO: cached property

  // Length
  length_property = desc.length ? propertyExpression(desc.length) : {
    get: function() { return this.byteLength * 8 - this[name + '__offset'] }
  }
  try { length = length_property.value || length_property.get.call(prototype) } catch(e) { length = undefined }
  if (typeof length === 'number' && !isNaN(length)) length_property = { value: length }
  Object.defineProperty(prototype, name + '__length', length_property)
  properties[name + '__length'] = length_property  // TODO: cached property
...
tools.structure = function(structure, name, prototype, previous) {
  ...
  if (name) {
    properties[name + '__offset'] = {
      get: function() { return this[keys[0] + '__offset'] },
      configurable: true
    }
    properties[name + '__length'] = {
