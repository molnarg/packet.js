...
function nestedStructure(structures, offset, name) {
  ...
  properties[name + '__offset'] = { value: offset }
  properties[name + '__length'] = { value: properties.bitLength.value }
...
tools.structure = function(structure, offsetStart) {
  ...
  keys.forEach(function(name, index) {
    ...
    if (desc instanceof Array) {
      ...
      properties[name] = cachedProperty(name, {
        get: function() { return desc.type(this.buffer, this.byteOffset + byteOffset, desc.length / 8) }
      })
      offset += desc.length
      ...
      properties[name + '__offset'] = { value: offset }
      properties[name + '__length'] = { value: desc.length }
      offset += desc.length


    } else {
      // Indefinite length field. This is exposed as a lazy initialized view.
      if (index !== keys.length - 1) throw new Error('Only the last field can be indefinitely long.')
      if (bitOffset !== 0) throw new Error('An indefinite length field must begin without bit offset.')

      properties[name] = cachedProperty(name, {
        get: function() { return tools.view(this.buffer, this.byteOffset + byteOffset) }
      })
      properties[name + '__offset'] = { value: offset }
      properties[name + '__length'] = cachedProperty(name + '__length', {
        get: function() { return  this.byteLength - (this.byteOffset + byteOffset) }
      })
    }
  })
