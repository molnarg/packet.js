...
function structurePropertyInitializer(desc, name) {
  var offset_name = name + '__offset', length_name = name + '__length'
  if (!(offset_name in this && length_name in this)) return undefined

  var length = this[length_name], offset = this[offset_name]

  if (length <= 32 && !desc.type) {
    if (desc.length === 1) {
      return tools.flag (Math.floor(offset / 8), offset % 8)
    } else {
      return tools.field(Math.floor(offset / 8), offset % 8, length, desc.signed, desc.littleEndian)
    }
    ...
    return {
      value: (desc.type || tools.view)(this.buffer, this.byteOffset + this[offset_name] / 8, this[length_name] / 8),
      configurable: true
    }
  }
}
...
function structureProperty(desc, name, prototype, previous) {
  ...
  var properties = {}, offset_name = name + '__offset', length_name = name + '__length'
  ...
  if ('value' in properties[offset_name]) prototype[offset_name] = properties[offset_name].value
  ...
  if ('value' in properties[length_name]) prototype[length_name] = properties[length_name].value
