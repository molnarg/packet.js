...
// Lazy initialization. Storing the initialized object in this[name]
// If name is the same name under which this property will be registered, it overwrites itself
// when initialized resulting in a simple value property
var counter = 0
function cachedProperty(init) {
  var name = 'cached_' + counter++

  return {
    get: function() {
      if (this[name]) return this[name]

      Object.defineProperty(this, name, { value: init.call(this) })
...
function nestedStructure(structures, offset, name) {
  ...
  properties[name] = cachedProperty(function() {
...
tools.structure = function(structure, offsetStart) {
  ...
  keys.forEach(function(name, index) {
    ...
    var byteOffset = Math.floor(offset / 8), bitOffset = offset % 8
    ...
    if (desc instanceof Array) {
      ...
      if (bitOffset !== 0) throw new Error('A typed field must begin without bit offset.')

      properties[name] = cachedProperty(function() {
        return desc.type(this.buffer, this.byteOffset + byteOffset, desc.length / 8)
      })
      offset += desc.length
      ...
      // Indefinite length field. This is exposed as a lazy initialized view.
      if (index !== keys.length - 1) throw new Error('Only the last field can be of indefinitely long.')
      if (bitOffset !== 0) throw new Error('An indefinite length field must begin without bit offset.')

      properties[name] = cachedProperty(function() {
        return tools.view(this.buffer, this.byteOffset + byteOffset)
      })
    }
  })
