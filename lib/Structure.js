...
function structurePropertyInitializer(desc, name) {
  ...
  if (length <= 32 && !type) {
    ...
    if (length === 1) {
      return fields.flag(offset / 8)

    } else {
      var property = fields.field(offset / 8, length, desc.signed, desc.littleEndian)
...
Structure.defineProperty = function(object, name, desc, previous) {
  if (name.indexOf('[') !== -1) {
    Structure.defineArrayProperty(object, name, desc, previous)

  } else if (desc instanceof Array && desc.length === 1) {
    Structure.defineBitfieldProperty(object, name, desc[0], previous)

  } else if (desc instanceof Array && desc.length > 1) {
    Structure.defineParallelProperties(object, name, desc, previous)

  } else {
    Structure.defineNestedProperties(object, name, desc, previous)
  }
}
